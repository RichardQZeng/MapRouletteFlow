...
public final class MapRouletteShortcuts {
    ...
    private static final Shortcut INSTRUCTIONS = Shortcut.registerShortcut("maprouletteflow:task",
            tr("MapRoulette Flow: Current task"), KeyEvent.CHAR_UNDEFINED, Shortcut.NONE);
    private static final Shortcut SELECT_PRIMITIVES = Shortcut.registerShortcut("maprouletteflow:select_task_primitives",
            tr("MapRoulette Flow: Select task primitives"), KeyEvent.CHAR_UNDEFINED, Shortcut.NONE);
    ...
    private static Map<CompletionResult, Shortcut> registerCompletionShortcuts() {
        ...
        for (var result : CompletionResult.values()) {
            ...
            shortcuts.put(result, Shortcut.registerShortcut("maprouletteflow:" + result.name().toLowerCase(java.util.Locale.ENGLISH),
                    tr("MapRoulette Flow: {0}", result.label()), key, modifiers));
        }
