// License: GPL. For details, see LICENSE file.
package io.github.richardqzeng.josm.maprouletteflow.api_caching;

import java.io.IOException;

import org.apache.commons.jcs3.access.CacheAccess;
import org.openstreetmap.josm.data.cache.JCSCacheManager;
import io.github.richardqzeng.josm.maprouletteflow.api.TaskAPI;
import io.github.richardqzeng.josm.maprouletteflow.api.model.Task;
import io.github.richardqzeng.josm.maprouletteflow.api.model.TaskClusteredPoint;
import io.github.richardqzeng.josm.maprouletteflow.data.IgnoreList;

/**
 * A cache for {@link Task} objects. Use when you don't need the absolute freshest data.
 */
public final class TaskCache {
    /**
     * The cache for task objects
     */
    private static final CacheAccess<Long, Task> CACHE = JCSCacheManager.getCache("maprouletteflow:task");

    private TaskCache() {
        // Hide constructor
    }

    /**
     * Cache results from {@link TaskAPI#get(long)}
     *
     * @param id The task id
     * @return The cached task
     * @throws IOException if there was a problem communicating with the server
     */
    public static Task get(long id) throws IOException {
        if (CACHE.get(id) == null) {
            synchronized (CACHE) {
                CACHE.put(id, TaskAPI.get(id));
            }
        }
        return CACHE.get(id);
    }

    /**
     * Check if we are ignoring/hiding a point
     *
     * @param point The point to check if we are ignoring
     * @return {@code true} if we are ignoring the point
     */
    public static boolean isHidden(TaskClusteredPoint point) {
        return IgnoreList.isChallengeIgnored(point.parentId()) || IgnoreList.isTaskIgnored(point.id())
                || ChallengeCache.isHidden(point.parentId());
    }
}
